...
export default async function handler(req: any, res: any) {
    try {
        if (req.method == "GET") {
            const querySnapshot = await getDocs(collection(db, "Consideracion"));
            let data: any = []
            querySnapshot.forEach(doc => data.push(doc.data()));
            res.status(200).json(data)
        }
        else if (req.method === 'POST') {
            const docRef = await addDoc(collection(db, "Consideracion"), req.body);
            ...
            const docRef = doc(db, 'Consideracion', req.body.ID);
