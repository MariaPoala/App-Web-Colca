...
export default async function handler(req: any, res: any) {
    try {
        if (req.method == "GET") {
            const querySnapshot = await getDocs(collection(db, "Tipo-Documento"));
            let data: any = []
            querySnapshot.forEach(doc => data.push(doc.data()));
            res.status(200).json(data)
        }
        else if (req.method === 'POST') {
            const docRef = await addDoc(collection(db, "Tipo-Documento"), req.body);
            ...
            const docRef = doc(db, 'Tipo-Documento', req.body.ID);
